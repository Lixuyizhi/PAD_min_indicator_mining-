...
class DominanceICAnalyzer:
    """
    支配维度因子IC分析器
    专门分析支配维度（Dominance）与未来收益率的相关性
    """
    ...
    def analyze_dominance(self):
        ...
        for return_col in returns:
            ic = self.calculate_ic('支配维度', return_col)
            self.ic_results[f"支配维度_vs_{return_col}"] = ic
            print(f"支配维度 vs {return_col}: IC = {ic:.4f}")
        
        return self.ic_results
    
    def plot_dominance_analysis(self):
        """绘制支配维度分析图表"""
        if not self.ic_results:
            print("请先运行 analyze_dominance()")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('支配维度因子IC分析', fontsize=16, fontweight='bold')
        ...
        # 2. 支配维度分布
        ax2 = axes[0, 1]
        dominance_data = self.data['支配维度'].dropna()
        ax2.hist(dominance_data, bins=50, alpha=0.7, color='lightgreen', edgecolor='black')
        ax2.set_title('支配维度分布')
        ax2.set_xlabel('支配维度')
        ax2.set_ylabel('频次')
        ax2.axvline(x=dominance_data.mean(), color='red', linestyle='--', 
                   label=f'均值: {dominance_data.mean():.2f}')
        ax2.axvline(x=0, color='blue', linestyle='--', alpha=0.5, label='中性线')
        ax2.legend()
        
        # 3. 滚动IC图
        ax3 = axes[1, 0]
        rolling_ic = self.calculate_rolling_ic('支配维度', 'FutureReturn_1period', window=50)
        ax3.plot(rolling_ic.index, rolling_ic.values, linewidth=1, alpha=0.7)
        ax3.set_title('支配维度滚动IC (50期窗口)')
        ax3.set_xlabel('时间')
        ...
        # 4. 散点图
        ax4 = axes[1, 1]
        valid_data = self.data[['支配维度', 'FutureReturn_1period']].dropna()
        ax4.scatter(valid_data['支配维度'], valid_data['FutureReturn_1period'], 
                   alpha=0.6, s=20, color='blue')
        ax4.set_title('支配维度 vs 1期收益率')
        ax4.set_xlabel('支配维度')
        ...
        # 添加趋势线
        z = np.polyfit(valid_data['支配维度'], valid_data['FutureReturn_1period'], 1)
        p = np.poly1d(z)
        ax4.plot(valid_data['支配维度'], p(valid_data['支配维度']), "r--", alpha=0.8)
        
        plt.tight_layout()
        plt.savefig('ic_analysis_plot/dominance_ic_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()  # 关闭图表，不显示
    ...
    def generate_report(self):
        ...
        print(f"支配维度统计:")
        dominance_stats = self.data['支配维度'].describe()
        print(dominance_stats)
        
        # 支配维度分布分析
        dominance_data = self.data['支配维度'].dropna()
        high_dominance_count = (dominance_data > dominance_data.quantile(0.75)).sum()
        low_dominance_count = (dominance_data < dominance_data.quantile(0.25)).sum()
        medium_dominance_count = len(dominance_data) - high_dominance_count - low_dominance_count
        
        print(f"\n支配维度分布:")
        print(f"高支配 (>75%分位数): {high_dominance_count} ({high_dominance_count/len(dominance_data)*100:.1f}%)")
        print(f"中等支配 (25%-75%分位数): {medium_dominance_count} ({medium_dominance_count/len(dominance_data)*100:.1f}%)")
        print(f"低支配 (<25%分位数): {low_dominance_count} ({low_dominance_count/len(dominance_data)*100:.1f}%)")
...
def main():
    """主函数"""
    analyzer = DominanceICAnalyzer('futures_emo_combined_data/sc2210_with_emotion_lag15min.xlsx')
    analyzer.load_data()
    analyzer.analyze_dominance()
    analyzer.plot_dominance_analysis()
