...
class SignalVolumeICAnalyzer:
    ...
    def analyze_signal_volume(self):
        """分析信号量因子"""
        print("开始分析信号量因子...")
        ...
        for return_col in returns:
            ic = self.calculate_ic('信号量', return_col)
            self.ic_results[f"信号量_vs_{return_col}"] = ic
            print(f"信号量 vs {return_col}: IC = {ic:.4f}")
        
        return self.ic_results
    
    def plot_signal_volume_analysis(self):
        """绘制信号量分析图表"""
        if not self.ic_results:
            print("请先运行 analyze_signal_volume()")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('信号量因子IC分析', fontsize=16, fontweight='bold')
        ...
        # 2. 信号量分布
        ax2 = axes[0, 1]
        signal_data = self.data['信号量'].dropna()
        ax2.hist(signal_data, bins=50, alpha=0.7, color='lightgreen', edgecolor='black')
        ax2.set_title('信号量分布')
        ax2.set_xlabel('信号量')
        ax2.set_ylabel('频次')
        ax2.axvline(x=signal_data.mean(), color='red', linestyle='--', 
                   label=f'均值: {signal_data.mean():.2f}')
        ax2.legend()
        
        # 3. 滚动IC图
        ax3 = axes[1, 0]
        rolling_ic = self.calculate_rolling_ic('信号量', 'FutureReturn_1period', window=50)
        ax3.plot(rolling_ic.index, rolling_ic.values, linewidth=1, alpha=0.7)
        ax3.set_title('信号量滚动IC (50期窗口)')
        ax3.set_xlabel('时间')
        ...
        # 4. 散点图
        ax4 = axes[1, 1]
        valid_data = self.data[['信号量', 'FutureReturn_1period']].dropna()
        ax4.scatter(valid_data['信号量'], valid_data['FutureReturn_1period'], 
                   alpha=0.6, s=20, color='blue')
        ax4.set_title('信号量 vs 1期收益率')
        ax4.set_xlabel('信号量')
        ax4.set_ylabel('1期收益率')
        ax4.grid(True, alpha=0.3)
        
        # 添加趋势线
        z = np.polyfit(valid_data['信号量'], valid_data['FutureReturn_1period'], 1)
        p = np.poly1d(z)
        ax4.plot(valid_data['信号量'], p(valid_data['信号量']), "r--", alpha=0.8)
        
        plt.tight_layout()
        plt.savefig('ic_analysis_plot/signal_volume_ic_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()  # 关闭图表，不显示
    ...
    def generate_report(self):
        ...
        print(f"信号量统计:")
        signal_stats = self.data['信号量'].describe()
        print(signal_stats)
...
def main():
    """主函数"""
    analyzer = SignalVolumeICAnalyzer('futures_emo_combined_data/sc2210_with_emotion_lag15min.xlsx')
    analyzer.load_data()
    analyzer.analyze_signal_volume()
    analyzer.plot_signal_volume_analysis()
