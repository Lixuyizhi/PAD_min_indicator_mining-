...
class TurtleBacktestRunner:
    ...
    def __init__(self, data_path, output_dir='turtle_results'):
        """
        初始化回测运行器
    ...
    def run_backtest(self, strategy_params=None):
        ...
        # 添加策略
        cerebro.addstrategy(TurtleStrategy, **default_params)
    ...
    def _save_parameters(self, params, data_info):
        ...
        param_file = os.path.join(self.output_dir, 
                                 f'turtle_params_{data_info["resample_rule"]}_lag{data_info["lag_minutes"]}min.txt')
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='纯海龟交易策略回测')
    parser.add_argument('--data_path', type=str, 
                       default='../futures_emo_combined_data/sc2210_with_emotion_1h_lag60min.xlsx',
                       help='数据文件路径 (默认: ../futures_emo_combined_data/sc2210_with_emotion_1h_lag60min.xlsx)')
    parser.add_argument('--output_dir', type=str, default='turtle_results',
                       help='输出目录')
    parser.add_argument('--entry_period', type=int, default=20,
                       help='入场通道周期')
    parser.add_argument('--exit_period', type=int, default=10,
                       help='出场通道周期')
    ...
    parser.add_argument('--take_profit', type=float, default=0.04,
                       help='止盈比例')
    parser.add_argument('--max_holding_periods', type=int, default=20,
                       help='最大持仓期数')
    ...
    # 创建回测运行器
    runner = TurtleBacktestRunner(args.data_path, args.output_dir)
